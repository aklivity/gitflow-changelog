...
function readDependencyArtifactIds(pomXml, groupId) {
  return asList(parsePom(pomXml).dependencies?.dependency).filter((dependency) => dependency.groupId === groupId).map((dependency) => dependency.artifactId).filter((artifactId) => artifactId !== void 0);
}
...
async function readModuleArtifactIds(gitDir) {
  const runtimeDir = join3(gitDir, "runtime");
  let moduleNames;
  try {
  ...
  const artifactIdsByDir = /* @__PURE__ */ new Map();
  for (const moduleName of moduleNames) {
    const pomPath = join3(runtimeDir, moduleName, "pom.xml");
    let pomXml;
    try {
      pomXml = await readFile3(pomPath, "utf8");
    } catch {
...
function moduleDirFromPath(path) {
...
async function filterForFoldIn(entries, options) {
  ...
  for (const entry of pullRequests) {
    ...
    const touchesDependency = paths.some((path) => {
      ...
      const artifactId = moduleDir ? options.moduleArtifactIds?.get(moduleDir) : void 0;
      return artifactId !== void 0 && (options.dependencySet?.has(artifactId) ?? false);
    });
...
async function computeUpstreamFoldIn(upstream, ownPlacement, options) {
  ...
  const dependencySet = upstream.classification === "maven" ? await readDependencySet(options.gitDir, upstream["maven-group-id"] ?? `io.aklivity.${upstreamRepo}`) : void 0;
  const moduleArtifactIds = upstream.classification === "maven" ? await readModuleArtifactIds(upstreamDir) : void 0;
