...
// src/types.ts
var EntryKind = external_exports.enum(["issue", "pr"]);
var Category = external_exports.enum(["enhancement", "bug", "issue", "excluded"]);
...
var UnresolvedReason = external_exports.enum(["not-found-anywhere", "ambiguous-candidates"]);
var UnresolvedEntry = external_exports.object({
  entry: Entry,
  reason: UnresolvedReason,
  candidates: external_exports.array(external_exports.string())
});
var Tag = external_exports.object({
  name: external_exports.string(),
  sha: external_exports.string(),
  date: external_exports.string()
});
var Bucket = external_exports.object({
  tag: Tag.nullable(),
  entries: external_exports.array(Entry)
});
var PlacementResult = external_exports.object({
  buckets: external_exports.array(Bucket),
  dropped: external_exports.array(Entry),
  unresolved: external_exports.array(UnresolvedEntry),
  ...
  allTags: external_exports.array(Tag)
});
var ClassificationLevel = external_exports.enum(["none", "path", "maven"]);
var PathClassification = external_exports.enum(["feature", "noise", "test-only"]);
var ExplicitUpstreamConfig = external_exports.object({
  repo: external_exports.string(),
  "dependency-version-file": external_exports.string(),
  "dependency-version-property": external_exports.string(),
  ...
  "maven-group-id": external_exports.string().optional()
});
var MavenDiscoveredUpstreamConfig = external_exports.object({
  maven: external_exports.object({
    groupId: external_exports.string(),
    artifactId: external_exports.string()
  })
});
var UpstreamConfig = external_exports.union([ExplicitUpstreamConfig, MavenDiscoveredUpstreamConfig]);
...
var RepoConfigSchema = external_exports.object({
  ...
  upstream: external_exports.array(UpstreamConfig).optional(),
  ...
  "mainline-branch": external_exports.string().optional(),
  "support-branch-pattern": external_exports.string().optional(),
  ...
  "exclude-paths": external_exports.array(external_exports.string()).optional(),
  "exclude-message-patterns": external_exports.array(external_exports.string()).optional(),
  "subject-match": external_exports.boolean().optional(),
  "subject-match-min-overlap": external_exports.number().optional(),
  ...
  "ports-trailer": external_exports.boolean().optional()
});
...
// src/merge-report-cli.ts
import { parseArgs } from "node:util";
import { writeFile as writeFile2 } from "node:fs/promises";

// src/merge-report-config.ts
var DEFAULT_EXCLUDE_PATHS = [".github/**", "CHANGELOG.md", ".gitflow-changelog*.yml"];
...
async function toMergeReportOptions(raw) {
  if (!raw.owner || !raw.repo) {
    throw new Error("owner and repo are required");
  }
  if (!raw.token) {
  ...
  const overlapInput = raw.subjectMatchMinOverlap ? Number(raw.subjectMatchMinOverlap) : void 0;
  return {
    owner: raw.owner,
    repo: raw.repo,
    token: raw.token,
    ...
    excludeLabels: splitList(raw.excludeLabels).length > 0 ? splitList(raw.excludeLabels) : fileConfig["exclude-labels"] ?? ["duplicate", "invalid", "wontfix"],
    ...
    excludeMessagePatterns: excludeMessagePatterns.length > 0 ? excludeMessagePatterns : fileConfig["exclude-message-patterns"] ?? [],
    subjectMatch: raw.subjectMatch !== void 0 ? parseBoolean(raw.subjectMatch, true) : fileConfig["subject-match"] ?? true,
    subjectMatchMinOverlap: overlapInput ?? fileConfig["subject-match-min-overlap"] ?? 0.3,
    portsTrailer: raw.portsTrailer !== void 0 ? parseBoolean(raw.portsTrailer, true) : fileConfig["ports-trailer"] ?? true
  };
}

// src/classification.ts
var DEFAULT_CLASSIFICATION_PATTERNS = {
  featurePaths: ["runtime/**/src/main/**", "specs/**", "incubator/**/src/main/**"],
  testPaths: ["**/src/test/**"]
};
function featurePathsFromModules(modules) {
...
function classifyPaths(paths, patterns = DEFAULT_CLASSIFICATION_PATTERNS) {
  if (paths.length === 0) {
    return "noise";
  }
...
var HashFallbackSchema = external_exports.object({
  originalSha: external_exports.string(),
  resolvedSha: external_exports.string()
});
var CacheFileSchema = external_exports.object({
  schemaVersion: external_exports.literal(CACHE_SCHEMA_VERSION),
  lastEventId: external_exports.number().int().nonnegative(),
  ...
  prFiles: external_exports.record(external_exports.string(), external_exports.array(external_exports.string())).optional().default({}),
  ...
  hashFallbacks: external_exports.record(external_exports.string(), HashFallbackSchema).optional().default({}),
  ...
  prBaseRefs: external_exports.record(external_exports.string(), external_exports.string()).optional().default({}),
  ...
  squashMergePrs: external_exports.record(external_exports.string(), external_exports.number()).optional().default({})
});
function emptyCache() {
  return {
...
function extractLastPage(linkHeader) {
  ...
  const match = linkHeader.split(",").map((part) => part.trim()).find((part) => part.endsWith('rel="last"'));
  if (!match) {
    return void 0;
  }
  ...
  return pageParam ? Number(pageParam) : void 0;
}
async function fetchEventsPage(owner, repo, page, token) {
...
function collectNew(events, lastEventId, seen, collected) {
  let addedAny = false;
...
async function walkRange(owner, repo, token, lastEventId, from, downTo, ceiling, seen, collected) {
  for (let page = from; page >= downTo; page -= 1) {
    const { events, lastPage } = await fetchEventsPage(owner, repo, page, token);
    if (lastPage !== void 0 && lastPage > ceiling.value) {
...
async function walkNewEvents(owner, repo, token, lastEventId) {
...
function applyEvent(cache, event) {
  const key = String(event.issue.number);
  const isPr = event.issue.pull_request !== void 0;
...
var CLOSING_KEYWORDS = /\b(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)\s*:?\s*#(\d+)/gi;
...
function applyClosingReferences(cache, events) {
  for (const event of events) {
    const isPr = event.issue.pull_request !== void 0;
...
function categorize(labels, options) {
...
function excludedShas(cache, excludeLabels) {
...
function entriesFromCache(cache, options) {
  const entries = [];
...
async function updateCache(cache, options) {
...
function isRateLimited(response) {
  ...
  return response.status === 403 && (response.headers.get("x-ratelimit-remaining") === "0" || response.headers.get("retry-after") !== null);
}
...
async function fetchPullRequestBaseRef(owner, repo, number, token) {
...
async function findSquashMergePr(owner, repo, headRef, baseRef, token) {
...
// src/ports-trailer.ts
var TRAILER_LINE = /^Ports:\s*([0-9a-fA-F]{7,40})\s*$/;
function parsePortsTrailers(body) {
  const values = [];
...
function portsTrailerMatches(candidateSha, trailerValues) {
...
async function runAllowFailure(args, options) {
  try {
...
async function showFile(ref, path, options) {
...
async function isExistingClone(dir) {
  try {
...
async function cloneOrUpdateRepo(owner, repo, dir, token) {
...
var FIELD_SEP = "";
var RECORD_SEP = "";
async function scanCommitsReferencingNumbers(ref, options) {
...
async function filesChangedInCommit(sha, options) {
...
async function unmatchedCommits(target, source, options) {
...
var SUBJECT_FIELD_SEP = "";
async function findCommitsContainingSubject(ref, substring, options) {
...
async function listCommitSubjects(range, options) {
...
async function resolveCommit(ref, options) {
...
var MergeIgnoreSchema = external_exports.object({
  "merge-ignore": external_exports.record(external_exports.string(), external_exports.string()).default({})
});
var EMPTY_MERGE_IGNORE = /* @__PURE__ */ new Map();
async function loadMergeIgnore(path) {
  if (!path) {
  ...
  const parsed = MergeIgnoreSchema.parse((0, import_yaml2.parse)(raw) ?? {});
  return new Map(Object.entries(parsed["merge-ignore"]));
}

// src/subject-match.ts
...
// src/topology.ts
function supportVersion(name, supportPattern) {
  const match = name.match(supportPattern);
  return match ? Number(match[1]) : void 0;
}
function computeTopology(branches, mainline, supportPattern) {
  const supportBranches = branches.map((name) => ({ name, version: supportVersion(name, supportPattern) })).filter((branch) => branch.version !== void 0).sort((a, b) => a.version - b.version);
  const topology = [];
...
async function discoverTopology(options) {
...
function isPathExcluded(files, excludePaths) {
  return files.length > 0 && excludePaths.length > 0 && files.every((file) => matchesAny(file, excludePaths));
}
async function isExcluded(candidate, targetRef, excludedLabelShas, ignoredShas, portsTrailerValues, options) {
...
async function mergeReport(options, now = /* @__PURE__ */ new Date()) {
...
// src/render/default.ts
var SPECIAL_CHARS = /[\\()[\]_*#]/g;
function escapeTitle(title) {
  return title.split(/(`[^`]*`)/).map((segment, index) => index % 2 === 1 ? segment : segment.replace(SPECIAL_CHARS, "\\$&")).join("");
}
...
function entryUrl(entry, options) {
...
function withSourceRepo(entry, repo) {
...
function tagDate(isoDate) {
  return isoDate.slice(0, 10);
}
function foldInNote(foldIn) {
  ...
  const url = foldIn.fromVersion ? `${repoUrl}/compare/${foldIn.fromVersion}...${foldIn.toVersion}` : `${repoUrl}/tree/${foldIn.toVersion}`;
  return `_Includes [${repoName} ${range}](${url})._`;
}
function renderBucket(bucket, previousTagName, options, foldIns) {
...
function render(placement, options, foldIns = /* @__PURE__ */ new Map()) {
...
// src/render/merge-report.ts
function commitUrl(sha, options) {
...
function renderMergeReport(result, options) {
...
// src/merge-report-cli.ts
async function runMergeReportCli(argv) {
  ...
  await writeFile2(values.output, markdown, "utf8");
}
...
// src/completeness.ts
var SQUASH_MERGE_SUFFIX = /\(#(\d+)\)\s*$/;
var MERGE_COMMIT_PREFIX = /^Merge pull request #(\d+) from /;
function extractMergedPrNumber(subject) {
  ...
  return merge ? Number(merge[1]) : void 0;
}
async function checkCompleteness(range, knownNumbers, gitOptions) {
  const commits = await listCommitSubjects(range, gitOptions);
  const issues = [];
  const seen = /* @__PURE__ */ new Set();
  for (const { sha, subject } of commits) {
    const number = extractMergedPrNumber(subject);
    if (number === void 0 || knownNumbers.has(number) || seen.has(number)) {
      continue;
    }
    seen.add(number);
...
function readDependencyVersion(pomXml, property) {
  const value = parsePom(pomXml).properties?.[property];
  return value === void 0 ? void 0 : String(value);
}
function readModuleArtifactId(pomXml) {
  return parsePom(pomXml).artifactId;
}
var EXCLUDED_SCOPES = /* @__PURE__ */ new Set(["test", "system"]);
function readDependencyEdges(pomXml, groupId) {
  return asList(parsePom(pomXml).dependencies?.dependency).filter((dependency) => dependency.groupId === groupId && dependency.artifactId !== void 0).map((dependency) => ({
    artifactId: dependency.artifactId,
    scope: dependency.scope ?? "compile",
    version: dependency.version === void 0 ? void 0 : String(dependency.version)
  })).filter((edge) => !EXCLUDED_SCOPES.has(edge.scope));
}
function readDependencyArtifactIds(pomXml, groupId) {
  return readDependencyEdges(pomXml, groupId).map((edge) => edge.artifactId);
}
var SKIP_DIRS = /* @__PURE__ */ new Set([".git", "target", "node_modules"]);
async function findPomFiles(dir) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      if (!SKIP_DIRS.has(entry.name)) {
        pomPaths.push(...await findPomFiles(join3(dir, entry.name)));
      }
    } else if (entry.name === "pom.xml") {
      pomPaths.push(join3(dir, entry.name));
    }
  }
  return pomPaths;
}
async function readDependencySet(gitDir, groupId) {
  const pomPaths = await findPomFiles(gitDir);
  const artifactIds = /* @__PURE__ */ new Set();
  for (const pomPath of pomPaths) {
    let pomXml;
    try {
      pomXml = await readFile4(pomPath, "utf8");
    } catch {
      continue;
    }
...
var PROPERTY_PLACEHOLDER = /^\$\{([^}]+)\}$/;
async function findGroupVersionSource(gitDir, groupId) {
  const pomPaths = await findPomFiles(gitDir);
  const rootPomPath = join3(gitDir, "pom.xml");
  const rootPomXml = await readFile4(rootPomPath, "utf8").catch(() => void 0);
  for (const pomPath of pomPaths) {
    let pomXml;
    try {
      pomXml = await readFile4(pomPath, "utf8");
    ...
    for (const edge of readDependencyEdges(pomXml, groupId)) {
      const property = edge.version === void 0 ? void 0 : PROPERTY_PLACEHOLDER.exec(edge.version)?.[1];
      if (property === void 0) {
        continue;
      }
      const file = relative(gitDir, pomPath).split(sep2).join("/");
      if (readDependencyVersion(pomXml, property) !== void 0) {
        return { file, property };
      }
      if (rootPomXml !== void 0 && readDependencyVersion(rootPomXml, property) !== void 0) {
...
async function indexMavenModules(gitDir, groupId) {
  const pomPaths = await findPomFiles(gitDir);
  const modules = [];
  for (const pomPath of pomPaths) {
    let pomXml;
    ...
    const dir = relative(gitDir, dirname(pomPath)).split(sep2).join("/");
    modules.push({ dir, artifactId, dependencies: readDependencyEdges(pomXml, groupId) });
  }
  return modules;
}
function resolveModule(path, modules) {
  let best;
  for (const module of modules) {
    const prefix = module.dir ? `${module.dir}/` : "";
    if (path.startsWith(prefix) && (best === void 0 || module.dir.length > best.dir.length)) {
      best = module;
    }
  }
  return best;
}
var PROPAGATING_SCOPES = /* @__PURE__ */ new Set(["compile", "runtime"]);
function expandTransitiveDependencySet(direct, modules) {
...
var DEFAULT_REGISTRY_URL = "https://maven.packages.aklivity.io";
function pomUrl(groupId, artifactId, version, registryUrl = DEFAULT_REGISTRY_URL) {
  const groupPath = groupId.replace(/\./g, "/");
  return `${registryUrl}/${groupPath}/${artifactId}/${version}/${artifactId}-${version}.pom`;
}
var MAX_REDIRECTS = 5;
async function fetchFollowingRedirects(url, init, redirectsLeft) {
...
async function fetchPom(groupId, artifactId, version, registryUrl = DEFAULT_REGISTRY_URL) {
  const response = await fetchFollowingRedirects(pomUrl(groupId, artifactId, version, registryUrl), {}, MAX_REDIRECTS);
  return response.ok ? response.text() : void 0;
}
var GITHUB_SCM_PATTERN = /github\.com[/:]+([^/]+)\/([^/.]+)(?:\.git)?\/?$/;
function extractGithubRepo(pomXml) {
  const doc = parser2.parse(pomXml);
  const scm = doc.project?.scm;
  if (scm === void 0) {
    return void 0;
  }
  const candidates = [scm.url, scm.connection, scm.developerConnection].filter((value) => value !== void 0).map(String);
  for (const candidate of candidates) {
...
// src/discover-upstream.ts
function isMavenDiscovered(upstream) {
  return "maven" in upstream;
}
async function resolveUpstreamConfig(upstream, gitDir) {
  ...
  if (source === void 0) {
    return {
      config: void 0,
      warning: `upstream maven ${label}: no dependency under groupId "${groupId}" with a property-pinned version was found in this checkout; skipping. (A literal, non-property <version> isn't supported for discovery yet \u2014 add an explicit repo/dependency-version-file/dependency-version-property entry instead.)`
    };
  }
  const pomXml = await readFile5(join4(gitDir, source.file), "utf8");
  const version = readDependencyVersion(pomXml, source.property);
  if (version === void 0) {
    return {
      config: void 0,
      warning: `upstream maven ${label}: property "${source.property}" was found in ${source.file} once but could not re-read; skipping.`
    };
  }
  const upstreamPomXml = await fetchPom(groupId, artifactId, version);
  const repo = upstreamPomXml === void 0 ? void 0 : extractGithubRepo(upstreamPomXml);
  if (repo === void 0) {
    return {
      config: void 0,
      warning: `upstream maven ${label}: could not discover a github.com <scm> URL from its published pom at version ${version}; skipping. (Non-GitHub or missing <scm> upstreams aren't supported yet \u2014 this tool's driver is GitHub-API-based.)`
    };
  }
  return {
    config: {
...
var OverridesSchema = external_exports.object({
  "hash-overrides": external_exports.record(external_exports.string(), external_exports.string()).default({})
});
var EMPTY_OVERRIDES = /* @__PURE__ */ new Map();
async function loadOverrides(path) {
  ...
  const parsed = OverridesSchema.parse((0, import_yaml3.parse)(raw) ?? {});
...
async function place(input, gitOptions) {
  ...
  const orderedSectionedTags = [...sectionedTags].map(toTag).sort((a, b) => b.date.localeCompare(a.date));
...
// src/resolve.ts
function fallbackCacheKey(entry) {
  return `${entry.kind}:${entry.number}`;
}
function emptySquashMergeCache() {
  return { prBaseRefs: {}, squashMergePrs: {} };
}
async function resolveBaseRef(entry, github, squashCache) {
...
async function resolveSquashMergePr(baseRef, ref, defaultBranch, github, squashCache) {
...
async function resolveViaSquashMerge(entry, ref, defaultBranch, github, squashCache, referencingByNumber) {
...
async function resolveHashes(input, gitOptions) {
  ...
  for (const entry of input.entries) {
    ...
    if (cached && cached.originalSha === entry.sha && await commitExists(cached.resolvedSha, gitOptions)) {
      warnings.push(
        `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository (likely history rewrite); reusing cached substitute ${cached.resolvedSha} from a previous run. If this is wrong, add an explicit override for ${entry.sha} to the changelog-hash-overrides file.`
      );
  ...
  const referencingByNumber = pending.length > 0 ? await scanCommitsReferencingNumbers(input.ref, gitOptions) : /* @__PURE__ */ new Map();
  const stillPending = [];
  for (const entry of pending) {
    ...
    if (candidates.length === 1) {
      ...
      warnings.push(
        `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository (likely history rewrite); auto-substituted ${candidate}, found by scanning commit messages for "#${entry.number}". If this recurs, add an explicit override for ${entry.sha} to the changelog-hash-overrides file.`
      );
  ...
  for (const entry of stillPending) {
    if (!rateLimited && input.github && defaultBranch && entry.kind === "pr") {
      try {
        ...
        if (squashCandidate) {
          warnings.push(
            `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository (likely squash-merged via a long-lived branch); auto-substituted ${squashCandidate.resolvedSha}, found via the squash-merge of base branch "${squashCandidate.baseRef}" in #${squashCandidate.squashPrNumber}. If this is wrong, add an explicit override for ${entry.sha} to the changelog-hash-overrides file.`
          );
  ...
  if (rateLimited) {
    warnings.push(
      "GitHub API rate limit hit while resolving squash-merge fallbacks; skipping this tier for the rest of this run rather than reporting the remaining entries as unresolvable. Any base refs and squash-merge PR numbers already discovered this run are still cached, so the next run resumes from there instead of starting over."
    );
  }
  const unresolved = [];
  for (const entry of unresolvedCandidates) {
    ...
    const candidateDescription = candidates.length === 0 ? "no candidate commit references it" : `${candidates.length} candidate commits reference it ambiguously`;
    warnings.push(
      `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository and ${candidateDescription}; dropping this entry. Add an explicit override for ${entry.sha} to the changelog-hash-overrides file to fix this.`
    );
...
// src/upstream.ts
async function readVersionAtRef(ref, file, property, gitOptions) {
  const pomXml = await showFile(ref, file, gitOptions);
  return pomXml === void 0 ? void 0 : readDependencyVersion(pomXml, property);
}
async function computeVersionRanges(placement, headRef, file, property, gitOptions) {
  const candidates = [
  ...
  for (const candidate of candidates) {
    ...
    const fromVersion = candidate.previousTag ? await readVersionAtRef(candidate.previousTag.name, file, property, gitOptions) : void 0;
...
async function placeUpstreamGlobally(entries, gitOptions) {
  const tagsByDateAsc = (await listTags(/.*/, gitOptions)).sort((a, b) => a.date.localeCompare(b.date));
  const entriesByTag = /* @__PURE__ */ new Map();
  for (const entry of entries) {
...
async function selectEntriesInRange(placement, fromVersion, toVersion, gitOptions) {
  ...
  const fromTagName = fromIndex !== -1 ? placement.tagsByDateAsc[fromIndex].name : void 0;
  const mergedIntoTo = await tagsMergedInto(toVersion, gitOptions);
  const mergedIntoFrom = fromTagName ? await tagsMergedInto(fromTagName, gitOptions) : void 0;
  const entries = [];
...
async function resolvePullRequestFiles(pullRequests, options) {
...
async function resolveIssuePaths(issues, pullRequests, pathsByNumber, options) {
  const prNumberBySha = /* @__PURE__ */ new Map();
  ...
  await forEachWithConcurrency(issues, FETCH_CONCURRENCY, async (issue) => {
    ...
    const paths = matchingPrNumber !== void 0 ? pathsByNumber.get(matchingPrNumber) ?? [] : await filesChangedInCommit(issue.sha, options.gitOptions);
    resolved.set(issue.number, paths);
  });
  return resolved;
}
async function filterForFoldIn(entries, options) {
  ...
  const patterns = options.patterns ?? (options.modules ? { featurePaths: featurePathsFromModules(options.modules), testPaths: DEFAULT_CLASSIFICATION_PATTERNS.testPaths } : DEFAULT_CLASSIFICATION_PATTERNS);
  const filtered = [];
  for (const entry of entries) {
    ...
    const touchesDependency = paths.some((path) => {
      const module = options.modules ? resolveModule(path, options.modules) : void 0;
      return module !== void 0 && (options.dependencySet?.has(module.artifactId) ?? false);
    });
...
async function computeFoldIn(options) {
...
function addFoldIn(byBucket, bucketTag, section) {
  const sections = byBucket.get(bucketTag) ?? [];
  sections.push(section);
  byBucket.set(bucketTag, sections);
}
function withFoldInOnlyBuckets(placement, foldIns) {
...
async function computeUpstreamFoldIn(upstream, ownPlacement, options) {
  const [upstreamOwner, upstreamRepo] = upstream.repo.split("/");
  const upstreamDir = join5(options.upstreamCacheDir, `${upstreamOwner}-${upstreamRepo}`);
  ...
  const upstreamOverrides = await loadOverrides(join5(upstreamDir, ".gitflow-changelog-hash-overrides.yml"));
  ...
  const modules = upstream.classification === "maven" ? await indexMavenModules(upstreamDir, groupId) : void 0;
  const directDependencySet = upstream.classification === "maven" ? await readDependencySet(options.gitDir, groupId) : void 0;
  const dependencySet = directDependencySet && modules ? expandTransitiveDependencySet(directDependencySet, modules) : directDependencySet;
...
async function determinePreviousTagName(placement, ref, gitOptions) {
...
async function run2(options) {
  ...
  const gitOptions = { cwd: options.gitDir };
  ...
  const foldIns = /* @__PURE__ */ new Map();
...
async function main() {
  ...
  const { markdown, warnings, completenessIssues } = await run2(options);
  for (const warning of warnings) {
    process.stderr.write(`warning: ${warning}
`);
  }
  await writeFile3(values.output, markdown, "utf8");
  if (completenessIssues.length > 0) {
    const details = completenessIssues.map((issue) => `#${issue.number} (${issue.sha})`).join(", ");
    process.stderr.write(
      `error: changelog completeness check failed: git history shows ${completenessIssues.length} merged PR(s) in this release that never appeared in the fetched entries \u2014 ${details}. ${values.output} was still written with whatever entries were found, but it is missing these.
`
    );
    process.exitCode = 1;
  }
