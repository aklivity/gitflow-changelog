...
import { dirname, join as join2, relative, sep as sep2 } from "node:path";
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      if (!SKIP_DIRS.has(entry.name)) {
        pomPaths.push(...await findPomFiles(join2(dir, entry.name)));
      }
    } else if (entry.name === "pom.xml") {
      pomPaths.push(join2(dir, entry.name));
    }
  }
  return pomPaths;
}
async function readDependencySet(gitDir, groupId) {
  ...
  for (const pomPath of pomPaths) {
    let pomXml;
    try {
      pomXml = await readFile3(pomPath, "utf8");
    } catch {
      continue;
    }
...
async function findGroupVersionSource(gitDir, groupId) {
  const pomPaths = await findPomFiles(gitDir);
  const rootPomPath = join2(gitDir, "pom.xml");
  const rootPomXml = await readFile3(rootPomPath, "utf8").catch(() => void 0);
  for (const pomPath of pomPaths) {
    let pomXml;
    try {
      pomXml = await readFile3(pomPath, "utf8");
...
async function resolveUpstreamConfig(upstream, gitDir) {
  ...
  const pomXml = await readFile4(join3(gitDir, source.file), "utf8");
...
var SPECIAL_CHARS2 = /[\\()[\]_*#]/g;
function escapeTitle(title) {
  return title.split(/(`[^`]*`)/).map((segment, index) => index % 2 === 1 ? segment : segment.replace(SPECIAL_CHARS2, "\\$&")).join("");
}
