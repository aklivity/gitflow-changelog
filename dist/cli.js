...
var UpstreamConfig = external_exports.object({
...
function readDependencyEdges(pomXml, groupId) {
  return asList(parsePom(pomXml).dependencies?.dependency).filter((dependency) => dependency.groupId === groupId && dependency.artifactId !== void 0).map((dependency) => ({ artifactId: dependency.artifactId, scope: dependency.scope ?? "compile" })).filter((edge) => !EXCLUDED_SCOPES.has(edge.scope));
}
...
async function computeUpstreamFoldIn(upstream, ownPlacement, options) {
  ...
  const upstreamDir = join4(options.upstreamCacheDir, `${upstreamOwner}-${upstreamRepo}`);
