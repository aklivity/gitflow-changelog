...
var EMPTY_OVERRIDES = {
  prOverrides: /* @__PURE__ */ new Map(),
  issueOverrides: /* @__PURE__ */ new Map()
};
...
async function resolveHashes(input, gitOptions) {
  ...
  for (const entry of input.entries) {
    ...
    if (cached && cached.originalSha === entry.sha && await commitExists(cached.resolvedSha, gitOptions)) {
      warnings.push(
        `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository (likely history rewrite); reusing cached substitute ${cached.resolvedSha} from a previous run. If this is wrong, add an explicit override for #${entry.number} to the changelog-hash-overrides file.`
      );
  ...
  for (const entry of pending) {
    ...
    if (candidates.length === 1) {
      ...
      warnings.push(
        `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository (likely history rewrite); auto-substituted ${candidate}, found by scanning commit messages for "#${entry.number}". If this recurs, add an explicit override for #${entry.number} to the changelog-hash-overrides file.`
      );
    ...
    warnings.push(
      `${entry.kind} #${entry.number}: recorded commit ${entry.sha} does not exist in this repository and ${candidateDescription}; dropping this entry. Add an explicit override for #${entry.number} to the changelog-hash-overrides file to fix this.`
    );
