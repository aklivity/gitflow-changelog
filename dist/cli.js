...
async function searchCommitsReferencing(issueNumber, ref, options) {
...
function selectUpstreamEntries(upstreamPlacement, fromVersion, toVersion) {
  const toIndex = upstreamPlacement.buckets.findIndex((bucket) => bucket.tag?.name === toVersion);
  if (toIndex === -1) {
    return [];
  }
  const fromIndex = fromVersion ? upstreamPlacement.buckets.findIndex((bucket) => bucket.tag?.name === fromVersion) : upstreamPlacement.buckets.length;
...
async function placeUpstreamAt(ref, upstreamEntries, upstreamTagPattern, upstreamGitOptions, cache) {
...
async function computeFoldIn(options) {
  ...
  const placementByVersion = /* @__PURE__ */ new Map();
