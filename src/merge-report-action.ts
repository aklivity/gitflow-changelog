...
async function main(): Promise<void> {
  ...
  await core.summary.addHeading(`merge-report: ${options.source}`).addRaw(markdown).write();

  const stale = result.outstanding.filter((entry) => entry.ageDays > failAfterDays);
  if (stale.length > 0)
  {
    core.setFailed(
      `${stale.length} commit(s) on "${options.source}" have been outstanding for more than ${failAfterDays} days ` +
